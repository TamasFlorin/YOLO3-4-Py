...
def build_darknet(darknet_dir, branch_name, target_location):
    ...
    response = requests.get(
        'https://github.com/pjreddie/darknet/archive/'+branch_name+'.zip')
    ...
    build_ret = subprocess.Popen("make", shell=True, stdout=subprocess.PIPE, cwd=os.path.join(
        download_path, "darknet-"+branch_name))
    ...
    shutil.move(os.path.join(download_path, "darknet-" +
                             branch_name+"/libdarknet.so"), target_location)
...
def find_site_packages():
    site_packages = [p for p in sys.path if p.endswith(
        "site-packages") or p.endswith("site-packages/")]
    return site_packages


def find_dist_packages():
    dist_packages = [p for p in sys.path if p.endswith(
        "dist-packages") or p.endswith("dist-packages/")]
